...
ANALYZE_PROMPT = """아파트·공동주택 관리 ERP 시스템(회계·부과·인사급여·검침·입주관리·전자결재·커뮤니티·민원·문의관리)을 위한 QA 작업입니다. 이 기획서를 분석하여 테스트해야 할 기능 목록을 추출해주세요.
...
TC_GENERATE_PROMPT = """아파트·공동주택 관리 ERP 시스템(회계·부과·인사급여·검침·입주관리·전자결재·커뮤니티·민원·문의관리)을 위한 QA 작업입니다. 경계값 분석·동등 분할·탐색적 테스트·결함 예측 기법을 적용하여 아래 기능에 대한 테스트케이스(TC)를 생성해주세요.
...
TREE_PROMPT = """아파트·공동주택 관리 ERP 시스템(회계·부과·인사급여·검침·입주관리·전자결재·커뮤니티·민원·문의관리)을 위한 QA 작업입니다. 이 기획서를 분석하여 테스트 대상 기능의 계층 구조(메뉴트리)를 추출해주세요.
...
STATE_INVENTORY_PROMPT = """아파트·공동주택 관리 ERP 시스템을 위한 QA 작업입니다. 기획서 전체를 읽고, TC 생성의 핵심 변수가 되는 "상태 차원(state dimensions)"을 도출하세요.
...
# ============================================================================
FLOW_PROMPT = """아파트·공동주택 관리 ERP 시스템을 위한 QA 작업입니다. 이 기획서를 읽고, 사람 QA가 테스트 설계로 쓰는 **상호작용 흐름 메뉴트리**를 만드세요.
단순 UI 요소 나열이 아니라 "어떤 상태에서, 무엇을 하면, 무엇이 표시되는가"의 **행동 흐름**을 좌→우로 펼친 트리입니다.

[노드 타입 (각 노드 type 1개 필수) — 사람(골든) 관례 기준. 의미·세부 작성 규칙은 아래 추가 지침 참고]
- PR = 사전조건/상태 | C = 클릭/선택/터치 | T = 입력 행위 | H = 마우스오버/툴팁
- DC = 더블클릭 | RC = 우클릭 | DD = Drag Down | D = 화면 표시/결과 | V = 수동 확인이 필요한 검증

[★ 정밀도 규칙 · 구조 배치 규칙 — 트리 문법]
표시 요소를 얼마나 잘게 쪼갤지, PR을 언제 새 경로로 분리할지, D 체이닝, 여집합 분기 등 트리 작성의
모든 세부 규칙은 룰셋의 "흐름 트리 정밀도 규칙" · "흐름 트리 구조 문법 규칙"(아래 추가 지침으로 주입됨)을
반드시 따른다 — 이 프롬프트에는 의도적으로 세부 규칙을 적지 않았다.

[★ JSON 출력 전 반드시 스스로 확인하라 — 이 체크리스트를 통과해야 출력 가능]
□ 팝업·바텀시트·페이지 자체를 "D: ○○ 팝업 출력", "D: ○○ 바텀시트 노출"처럼 컨테이너 한 줄로 쓴 곳이 있는가?
  → 있다면 팝업 내부의 타이틀·버튼·텍스트를 각각 개별 D로 분해하고, 클릭 버튼들은 팝업을 연 C의 형제로 이동하라.
□ PR content에 "~에 진입된 상태", "~이 노출된 상태"처럼 이미 도착한 결과가 담긴 곳이 있는가?
  → 있다면 "~하기 직전 상태"로 고치고, 그 행동을 C 노드로 추가하라.
□ C 노드가 D 노드의 children 안에 들어가 있는 곳이 있는가?
  → 있다면 그 C를 D의 부모(팝업을 연 C)의 형제로 꺼내라.
□ 클릭 C 노드 바로 앞에 그 버튼/체크박스가 화면에 표시됨을 나타내는 D 노드가 있는가?
  → 없다면 "D: ○○ 버튼" 또는 "D: ○○ 체크박스" D 노드를 C 앞에 추가하라.
□ 모든 노드에 spec_page 필드가 있는가?
  → 없다면 기획서 페이지 번호를 채우고, 모르면 빈 문자열("")을 넣어라.

[출력 — 순수 JSON만, 코드블록 없이. 아래는 스키마 형태 예시일 뿐 내용을 그대로 베끼지 마라]
{
  "title": "기획서 제목",
  "tree": [
    {"type":"PR","content":"<상위 화면에 진입된 상태 — 다음 화면으로 전환되기 '이전' 상태>","menu_path":"<메뉴 경로>","spec_page":"5","children":[
      {"type":"C","content":"<하위 화면/항목 선택 — 화면 전환을 일으키는 행동을 빠짐없이 명시>","spec_page":"5","children":[
        {"type":"D","content":"<전환 후 첫 번째 표시 요소>","spec_page":"5","children":[
          {"type":"D","content":"<같은 진입에서 이어지는 다음 표시 요소 — 새 경로 만들지 말고 체이닝>","spec_page":"5","children":[
            {"type":"C","content":"<해당 화면 내 버튼 클릭 등 또 다른 행동>","spec_page":"6","children":[
              {"type":"D","content":"<결과 — 기획서 문구 그대로 인용>","spec_page":"6","children":[]}
            ]}
          ]}
        ]}
      ]}
...
def _summarize_menu_tree_screens(menu_tree: Dict) -> str:
    """구조적 트리(menu_tree)를 '화면 > [기능들]' 텍스트 목록으로 평탄화.

    흐름 트리 생성 시 화면 골격으로 주입해, 같은 화면이 루트로 중복 생성되는 것을 방지하는 데 쓴다.
    리프 자식을 가진 노드를 "화면"으로 간주(트리 깊이가 2~4단계로 들쭉날쭉해도 동작).
    """
    lines: List[str] = []
    ...
    def walk(node: Dict, path: List[str]):
        ...
        leaf_children = [c for c in children if not (c.get("children") or [])]
        branch_children = [c for c in children if c.get("children")]
        if leaf_children:
            lines.append(" > ".join(cur_path))
            for lc in leaf_children:
                lines.append(f"  - {lc.get('name', '')}")
        for bc in branch_children:
            walk(bc, cur_path)

    for root in (menu_tree or {}).get("tree", []) or []:
        walk(root, [])
    return "\n".join(lines)


def build_flow_tree(pdf_path: str, ruleset=None, original_filename: str = None, menu_tree: Dict = None) -> Dict:
    ...
    menu_tree가 주어지면 그 화면 계층을 골격으로 주입해, 같은 화면이 흐름 트리에서
    중복 루트로 쪼개지지 않도록 한다(화면 단위 = build_menu_tree의 리프 부모 노드).
    """
    ...
    # 구조 문법(PR/C/D 배치)은 룰셋이 비어있어도 항상 주입 — 트리 문법이 없으면 AI가 구조를 임의로 만들어 버그가 남
    flow_rules = (ruleset.flow_rules if ruleset and ruleset.flow_rules else DEFAULT_FLOW_RULES)
    extra_parts.append(f"\n\n[추가 지침 — 반드시 따라야 할 구조 문법]\n{flow_rules}")

    screens_text = _summarize_menu_tree_screens(menu_tree) if menu_tree else ""
    if screens_text:
        extra_parts.append(
            "\n\n[화면 골격 — 이미 확정된 구조적 트리. 절대 규칙으로 따를 것]\n"
            "아래는 이 기획서에서 이미 추출된 화면 계층 구조다. 각 화면(아래 목록의 각 그룹, "
            "'경로 > [하위 기능들]' 한 묶음)마다 흐름 트리에서 **정확히 하나의 사전조건(루트 PR)만** 만들어라. "
            "같은 화면에 속한 여러 기능(하위 항목들)은 그게 몇 개든 전부 그 하나의 루트 아래 형제 분기로 "
            "통합하고, 같은 화면에 대해 새 루트를 또 만들지 마라(예: '팝업 노출 여부'와 'QR버튼 항상 표시 "
            "여부'가 같은 화면의 별개 기능이어도 같은 루트의 형제 분기여야 한다). 아래 목록에 없는 화면을 "
            "새로 추가하지 마라. 단, 화면 전환 중 나타나는 팝업·바텀시트·하위 페이지 등 흐름 내부 산출물은 "
            "트리 깊은 곳에 자연스럽게 포함해도 된다(이런 것들은 화면 목록에 안 보여도 된다).\n\n"
            f"{screens_text}"
        )

    prompt = FLOW_PROMPT + "".join(extra_parts)
...
COVERAGE_CHECK_PROMPT = """아래 [QA 규칙]을 기준으로 [흐름 트리]를 점검하여, **규칙을 충족하지 못한(누락·위반) 항목만** 찾아내세요.

원칙:
- 규칙에 부합하는 부분은 보고하지 마세요. 누락/위반만.
- 흐름 트리에 실제로 없는 것을 근거로만 지적하세요. 추측·일반론 금지.
- 규칙이 트리 구조와 무관하면 무시하세요.
- **조건부 규칙 주의(중요)**: "기획서가 명시한 경우에만", "명시되어 있다면" 같은 조건부 규칙은, 트리에 그 정보가 실제로 있는데 누락·오태깅된 경우에만 위반으로 보고하세요. 트리에 단서가 전혀 없으면 기획서에 애초에 정보가 없는 것일 수 있으므로 **위반으로 보고하지 마세요.** (굳이 보고한다면 반드시 fixability=spec_limited)
- 입력란의 허용문자·길이·필수여부·플레이스홀더, 특정 메시지 원문, 권한별 분기 등은 기획서에 명시되지 않는 경우가 많습니다. 트리에 그 단서(플레이스홀더 D, 형식 안내 문구 등)가 없으면 **not_followed로 분류하지 말고 spec_limited로만** 처리하거나 보고하지 마세요. 같은 항목을 반복해서 위반으로 만들지 마세요.
- **흐름 트리에서 상태(권한·코드·설정·데이터 존재 등)를 PR 노드로 분기하는 것은 올바른 설계입니다.** "권한 등 상태를 트리 노드로 두지 말고 TC 사전조건으로만 표현하라" 같은 (구 구조적 트리용) 규칙은 흐름 트리에 적용되지 않으므로, 트리에 상태 PR이 있다는 이유로 위반을 보고하지 마세요.

[각 finding의 fixability(해결 가능성)를 반드시 판정]
- "not_followed": 룰셋엔 해당 기대가 있는데 트리가 따르지 않음. 그 정보가 기획서에 있었다면 트리에 들어갔을 종류(일반적으로 기획서에 명시되는 화면 요소·동작·문구). → 룰셋 규칙을 더 명확히 강화하거나 재추출하면 개선 가능.
- "spec_limited": 트리에 없는 이유가 기획서에 그 정보(예: 입력 형식·길이 제약, 특정 메시지 문구, 권한별 분기 등)가 애초에 없기 때문으로 보임. 추측으로 만들 수 없어 룰셋에 추가해도 채워지지 않는 종류. → 기획서 보강이 필요.
- 판단 기준: 그 항목이 보통 기획서에 명시되는 종류인가? 트리의 인접 노드에 그 정보의 단서가 있는가? 단서가 전혀 없으면 spec_limited.

[QA 규칙 — target=flow와 target=tree 두 섹션으로 구분되어 있음]
{rules}
...
다음 JSON으로만 응답 (코드블록 없이):
{
  "findings": [
    {
      "rule": "관련 규칙 요약(한 줄)",
      "target": "flow 또는 tree — 이 finding이 어느 섹션의 규칙을 위반/누락했는지 (위 [QA 규칙]의 target= 라벨 그대로)",
      "severity": "missing 또는 violation",
      "fixability": "not_followed 또는 spec_limited",
      "where": "관련 화면/노드 경로(있으면)",
      "detail": "무엇이 빠졌거나 어긋났는지 구체적으로",
      "suggestion": "not_followed면 룰셋에 추가할 강화된 규칙 형태로, spec_limited면 기획서에 어떤 정보가 보강돼야 하는지"
    }
  ]
}
...
REPAIR_PROMPT = """아래 [발견된 위반·누락] 목록의 문제들을 [기획서 내용]을 참고하여 [현재 흐름 트리]에서 수정하고, 수정된 전체 트리를 출력하세요.

수정 원칙:
- 목록에 명시된 위반·누락만 고쳐라. 나머지 노드는 그대로 유지하라.
- **기획서에 근거가 있는 수정만 하라.** 팝업·바텀시트를 분해할 때는 기획서에 명시된 실제 UI 요소(타이틀·버튼명·텍스트 등)를 그대로 인용하라. 기획서에 없는 내용을 추측으로 추가하지 마라.
- 수정 후에도 전체 트리의 JSON 구조(type/content/children/spec_page/menu_path/id 필드)를 반드시 유지하라.
- spec_page는 기획서에서 해당 내용이 나온 실제 페이지 번호를 적어라(모르면 "").
- "title" 필드도 원본 그대로 유지하라.
...
출력: 수정된 전체 트리 JSON만, 코드블록 없이."""


def repair_flow_tree(flow_tree: Dict, findings: List[Dict],
                     pdf_path: str = None, original_filename: str = None) -> Dict:
    """커버리지 점검의 not_followed 위반을 Gemini 2차 패스(+기획서 참조)로 자동 교정한다.

    pdf_path가 제공되면 기획서 내용을 프롬프트에 포함해, 팝업 분해·spec_page 보완 등
    기획서 정보가 필요한 위반도 교정할 수 있다.
    실패하거나 수정할 내용이 없으면 원본 flow_tree를 그대로 반환한다.
    """
    fixable = [f for f in findings if f.get("fixability") == "not_followed"]
    if not fixable:
        return flow_tree

    findings_text = "\n".join(
        f"- [{f.get('severity','violation')}] {f.get('detail', f.get('rule',''))}"
        + (f" (제안: {f['suggestion']})" if f.get("suggestion") else "")
        for f in fixable
    )
    tree_json = json.dumps(flow_tree, ensure_ascii=False)
    ...
    try:
        if pdf_path and is_text_based_pdf(pdf_path):
            spec_text = extract_text_from_pdf(pdf_path)
            if len(spec_text) > 18000:
                spec_text = spec_text[:18000] + "\n\n[이하 생략]"
            full_prompt = f"{filename_hint}[기획서 내용]\n{spec_text}\n\n{base_prompt}"
            response_text = _generate_content_with_retry(
                full_prompt,
                model=settings.GEMINI_MODEL_EXTRACT,
                fallback_model=settings.GEMINI_MODEL_EXTRACT_FALLBACK or None,
            ...
            chunks = pdf_to_base64_chunks(pdf_path, chunk_size=10)
            contents = [{"inline_data": {"mime_type": "application/pdf", "data": b}} for b in chunks]
            contents.append(filename_hint + base_prompt if filename_hint else base_prompt)
            response_text = _generate_content_with_retry(
                contents,
                model=settings.GEMINI_MODEL_EXTRACT,
                fallback_model=settings.GEMINI_MODEL_EXTRACT_FALLBACK or None,
        ...
        repaired = _parse_json_response(response_text)
        if repaired and isinstance(repaired, dict) and repaired.get("tree"):
            _assign_flow_ids(repaired.get("tree", []))
            return repaired
        print("[흐름트리 교정] 교정 결과 파싱 실패 — 원본 유지")
    except Exception as e:
        print(f"[흐름트리 교정 실패] {str(e)[:150]} — 원본 유지")
    return flow_tree


FLOW_TC_ENRICH_PROMPT = """아파트·공동주택 관리 ERP 시스템을 위한 QA 작업입니다.
아래 [흐름 경로 목록]은 기획서에서 추출한 사용자 상호작용 시나리오들입니다.
각 경로에 대해 테스터가 바로 수행할 수 있는 자연어 TC를 1건씩 작성하세요.

작성 원칙:
- **사전조건**: PR 노드들의 상태를 자연스러운 한 문장으로 결합 (예: "로그인 후 미납대장 화면에 접속한 상태")
- **테스트 스텝**: C/T 노드들을 순서 있는 수행 절차로 작성. 각 스텝 다음 줄에 "  → 기대 표시: " 형태로 D/V 결과 명시
- **기대결과**: 마지막 D/V 노드의 내용을 "~해야 한다" / "~이 표시되어야 한다" 형태의 명확한 판단 기준으로 작성
- **제목**: 이 시나리오를 한 줄로 요약 (동작 + 기대 결과)
- 노드의 실제 문구·버튼명·메시지를 그대로 인용하라 (이미 기획서에서 추출된 원문이다)
- "정상적으로 표시된다" 같은 추상적 표현 금지. 구체적 문구로 작성하라
- spec_page: 경로 내 spec_page 값을 그대로 사용 (없으면 "")

[흐름 경로 목록] (row번호: 노드타입[내용] 순서)
{paths}

다음 JSON 형식으로만 응답 (코드블록 없이):
{{
  "tcs": [
    {{
      "row": <row번호(정수)>,
      "title": "시나리오 제목",
      "preconditions": "자연어 사전조건",
      "steps": "1. 수행 단계\\n   → 기대 표시: ...\\n2. 수행 단계\\n   → 기대 표시: ...",
      "expected_result": "최종 기대결과 (구체적 문구)",
      "spec_page": ""
    }}
  ]
}}"""

_FLOW_TC_BATCH_SIZE = 30  # 한 번 Gemini 호출에 처리할 경로 수


def generate_tcs_from_flow_paths(path_tracker: Dict) -> Dict[int, Dict]:
    """흐름트리 경로(path_tracker)를 Gemini로 자연어 TC로 변환한다 (PDF 없이, 배치 처리).

    path_tracker: {row_number: [node, ...]} — render_flow_tree_excel의 _out_paths 결과.
    경로 내용(노드 content)이 이미 기획서에서 추출된 원문이므로 PDF 재전송 불필요.
    _FLOW_TC_BATCH_SIZE 단위로 나눠 Gemini를 여러 번 호출해 대용량 문서도 처리 가능.
    반환: {row_number: tc_dict}
    """
    if not path_tracker:
        return {}

    def _path_to_text(row_num: int, path: List[Dict]) -> str:
        parts = []
        spec_page = ""
        for n in path:
            t = n.get("type", "?")
            c = (n.get("content") or "").replace("\n", " ")[:100]
            parts.append(f"{t}[{c}]")
            if not spec_page and n.get("spec_page"):
                spec_page = n["spec_page"]
        sp = f" (spec_page={spec_page})" if spec_page else ""
        return f"row={row_num}{sp}: " + " → ".join(parts)

    sorted_rows = sorted(path_tracker.keys())
    batches = [sorted_rows[i:i + _FLOW_TC_BATCH_SIZE]
               for i in range(0, len(sorted_rows), _FLOW_TC_BATCH_SIZE)]

    result: Dict[int, Dict] = {}
    for b_idx, batch_rows in enumerate(batches):
        paths_text = "\n".join(
            _path_to_text(r, path_tracker[r]) for r in batch_rows
        )
        prompt = FLOW_TC_ENRICH_PROMPT.replace("{paths}", paths_text)
        print(f"[흐름TC생성] 배치 {b_idx+1}/{len(batches)}: row {batch_rows[0]}~{batch_rows[-1]} ({len(batch_rows)}건)")
        try:
            ...
            parsed = _parse_json_response(resp)  # {"tcs": [...]} 형태
            tcs = parsed.get("tcs", []) if isinstance(parsed, dict) else []
            if not tcs:
                print(f"  [파싱 실패] 배치 {b_idx+1} 스킵")
                continue
            for tc in tcs:
                row = tc.get("row")
                if row is not None:
                    result[int(row)] = tc
            print(f"  → {len(tcs)}건 생성 (누적 {len(result)}건)")
        except Exception as e:
            print(f"  [배치 실패] {str(e)[:150]} — 이 배치 스킵")
            continue

    print(f"[흐름TC생성 완료] 총 {len(result)}건")
    return result
