...
| Method | URL | 설명 |
|--------|-----|------|
| POST | /api/documents/{id}/flow-tree | 흐름 트리 추출 시작 (백그라운드) |
| GET | /api/documents/{id}/flow-tree | 흐름 트리 + 통계 조회 (ready 여부) |
| GET | /api/documents/{id}/flow-tree/export | 흐름 트리 Excel(사람 양식) 다운로드 |
| POST | /api/documents/{id}/flow-tree/generate-tc | 흐름 트리 선형화 → TC 생성(기존 TC 교체) |
| POST | /api/documents/{id}/flow-tree/coverage-check | 룰셋 대비 누락·위반 점검 (fixability 분류) |
| POST | /api/documents/{id}/ruleset/append-rule | 규칙을 룰셋에 추가 (시스템 룰셋은 clone-on-write) |
...
| Phase 5.3 | 사람 골든 패턴 기반 룰셋 시딩 (docs/mandocs 4종 분석 → tree_rules 초안 추출) | ⏳ 진행 중 |
| Phase 4.11 | TC 분류·생성 결과 정량 재검증 (사람 엑셀 vs 새 AI 결과) | ⏳ 예정 |
| Phase 5 | 프론트+백 통합 및 최적화 | ⏳ 예정 |
