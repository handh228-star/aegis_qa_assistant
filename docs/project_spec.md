...
- 상태 분기: **트리 노드 분리 대신 TC 사전조건으로 표현** (이중 분리 방지)
...
| **Phase 5.0** | **흐름 트리 정식 전환** (스키마·추출·Excel·TC 선형화·편집UI) → [설계서](flow_tree_schema_design.md) | 🚧 설계 완료, 구현 착수 |
| **Phase 5.1** | **QA 피드백 흡수 시스템** (인스턴스/규칙 분리·골든 비교) → [설계서](feedback_system_design.md) | 📐 설계 완료 |
